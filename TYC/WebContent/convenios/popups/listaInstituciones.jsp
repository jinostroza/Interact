...
var listadoEmpresasInstData = null;
	...
	$(function() {
		...
		$("#listadoEmpresasInst").jqGrid({
			...
			colModel : [
						...
						{name : 'rut',index:'rut',	width : 240, resizable : false,	sortable : true},  
			 			{name : 'nombre',index:'nombre', width : 520, search : true, resizable : false, sortable : true}
						],
			xmlReader : {
				root : 'filas',
				row : 'fila',
			...
			caption : '${solemfmt:getLabel("label.InstitucionesPrestadoras", codIdioma)}',
			width : 770,
			height : 155,
			scrollOffset : 0,
			rowNum : 0,
			hoverrows : true,  
			loadtext : '${solemfmt:getLabel("label.BuscandoPrestadores", codIdioma)}',
			emptyrecords : '${solemfmt:getLabel("label.NoEncontraronPrestadores", codIdioma)}',
			hidegrid : false,
			sortname : 'run',  
			sortorder : 'asc',
			page : 1,
			...
			onPaging: function(pgButton){
				pagUsuario = $("#listadoEmpresasInst").jqGrid('getGridParam','page');
				totalRegistros = $("#listadoEmpresasInst").jqGrid('getGridParam','records');
				regPorPagina = $("#listadoEmpresasInst").jqGrid('getGridParam','rowNum');
				totalPaginas = Math.ceil(totalRegistros/regPorPagina);

				if(totalPaginas<pagUsuario)
				{
					$("#listadoEmpresasInst").setGridParam({page:totalPaginas});				
				}
			},
	...
	function setearInstitucion(idEmpresaInst, nomInstitucion){
		window.parent.$('#conIdPres').val(idEmpresaInst);
		window.parent.$('#conNomPres').val(nomInstitucion);
	}
</script> 
