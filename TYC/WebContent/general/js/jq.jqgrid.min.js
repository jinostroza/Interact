	...
	b.extend(b.jgrid, {
		...
		parseDate : function(f, j)
		{
			...
			if (j && j !== null && j !== undefined)
			{
				...
				j = j.split(/[\\\/:_;.\t\T\s-]/);
				f = f.split(/[\\\/:_;.\t\T\s-]/);
				var l = b.jgrid.formatter.date.monthNames, a = b.jgrid.formatter.date.AmPm, r = function(u, B)
				{
					if (u === 0)
					{
						if (B == 12)
		...
		from : function(f)
		{
			return new (function(j, i)
			{
				...
				this._getGroup = function(n, m, p, A, t)
				{
					...
					b.each(c._getOrder(n, m, p, A, t), function(fa, ca)
					{
				...
				{
					return c._compareValues(c.equals, n, m, "==", p)
				};
				this.greater = function(n, m, p)
				{
					return c._compareValues(c.greater, n, m, ">", p)
				};
				this.less = function(n, m, p)
				{
					return c._compareValues(c.less, n, m, "<", p)
				};
				this.greaterOrEquals = function(n, m, p)
				{
					return c._compareValues(c.greaterOrEquals, n, m, ">=", p)
				};
				this.lessOrEquals = function(n, m, p)
				{
					return c._compareValues(c.lessOrEquals, n, m, "<=", p)
				};
				this.startsWith = function(n, m)
				{
					var p = m === undefined || m === null ? n : m;
					p = l ? b.trim(p.toString()).length : p.toString().length;
					if (M)
						c._append(c._getStr("this." + n) + ".substr(0," + p + ") == " + c._getStr('"' + c._toStr(m) + '"'));
					else
					{
						p = l ? b.trim(m.toString()).length : m.toString().length;
				...
				this.endsWith = function(n, m)
				{
					var p = m === undefined || m === null ? n : m;
					p = l ? b.trim(p.toString()).length : p.toString().length;
					M ? c._append(c._getStr("this." + n) + ".substr(" + c._getStr("this." + n) + ".length-" + p + "," + p + ') == "' + c._toStr(m) + '"') : c
							._append(c._getStr("this") + ".substr(" + c._getStr("this") + '.length-"' + c._toStr(n) + '".length,"' + c._toStr(n)
									+ '".length) == "' + c._toStr(n) + '"');
				...
				{
					M ? c._append(c._getStr("this." + n) + '.indexOf("' + c._toStr(m) + '",0) > -1') : c._append(c._getStr("this") + '.indexOf("' + c._toStr(n)
							+ '",0) > -1');
				...
				.each(function()
				{
					if (!this.grid)
					{
						...
						else
						{
							...
							{
								var r = b("<div class='ui-jqgrid-view'></div>"), u, B = b.browser.msie ? true : false, G = b.browser.safari ? true : false;
								a = this;
								...
								/*TODO:solem: se comenta el div que mostraba el mensaje de cargando de la grilla */
								/*b("<div class='loading ui-state-default ui-state-active' id='load_" + this.id + "'>" + this.p.loadtext + "</div>")
										.insertBefore(r);*/
								b(this).attr({
									cellSpacing : "0",
									cellPadding : "0",
								...
								}, m = function(d, g, h, q, o)
								{
									...
									if (typeof h.formatter !== "undefined")
									{
										...
										g = b.isFunction(h.formatter) ? h.formatter.call(a, g, d, q, o) : b.fmatter ? b.fn.fmatter(h.formatter, g, d, q, o)
												: n(g)
									} else
										g = n(g);
									return g
								...
								}, t = function(d, g, h, q)
								{
									...
									d = M(d, g, "");
									return '<td role="gridcell" aria-describedby="' + a.p.id + '_rn" class="ui-state-default jqgrid-rownum" ' + d + ">" + h
											+ "</td>"
								}, H = function(d)
								{
									var g, h = [], q = 0, o;
								...
								{
									var x = new Date, w = a.p.datatype != "local" && a.p.loadonce || a.p.datatype == "xmlstring", C, D = a.p.datatype == "local" ? "local"
											: "xml";
									if (w)
									{
										a.p.data = [];
									...
									if (b.isXMLDoc(d))
									{
										...
										if (d && ga)
										{
											...
											for (; $ < ga;)
											{
												...
												s = h === 0 ? 0 : h + 1;
												s = (s + $) % 2 == 1 ? oa : "";
												X.push("<tr" + xa + ' id="' + L + '" role="row" class ="ui-widget-content jqgrow ui-row-' + a.p.direction + ""
														+ s + '">');
												...
												if (a.p.grouping)
												{
													...
													for ( var ya = 0; ya < s; ya++)
														E.push(T[a.p.groupingView.groupField[ya]]);
												...
												if (a.p.gridview === false)
												{
													...
													o && a.p.afterInsertRow.call(a, L, T, N);
													X = []
												}
												T = {};
												v++;
									...
									if (d)
									{
										...
										if (T = b.jgrid.getAccessor(d, g.root))
										{
											...
											for (; d < V;)
											{
												...
												v = h === 1 ? 0 : h;
												v = (v + d) % 2 == 1 ? X : "";
												E.push("<tr" + ra + ' id="' + L + '" role="row" class= "ui-widget-content jqgrow ui-row-' + a.p.direction + ""
														+ v + '">');
												if (a.p.rownumbers === true)
												{
											...
											if (a.p.gridview === true)
												a.p.grouping && F ? b(a).jqGrid("groupingRender", ha, a.p.colModel.length) : b("#" + a.p.id + " tbody:first")
														.append(E.join(""));
											a.p.totaltime = new Date - x;
											if (s > 0)
												if (a.p.records === 0)
									...
									if (b.isArray(a.p.data))
									{
										...
										b.each(a.p.colModel, function()
										{
											...
											if (x == "date" || x == "datetime")
											{
												...
												{
													o = this.formatoptions && this.formatoptions.srcformat ? this.formatoptions.srcformat
										...
										else
										{
												...
												if (v)
												{
													...
													for ( var E = 0, I = v.rules.length, F; E < I; E++)
													{
														...
														if (D[F.op] && F.field && F.data && y)
															s = y.toUpperCase() == "OR" ? D[F.op](s)(F.field, F.data, h[F.field]).or() : D[F.op](s)(F.field,
																	F.data, h[F.field])
													}
												} else
													try
													{
														s = D[a.p.postData.searchOper](s)(a.p.postData.searchField, a.p.postData.searchString,
											...
											if (d && a.p.sortorder && g)
												a.p.sortorder.toUpperCase() == "DESC" ? s.orderBy(a.p.sortname, "d", h[d].stype, h[d].srcfmt) : s.orderBy(
														a.p.sortname, "a", h[d].stype, h[d].srcfmt);
											D = s.select();
									...
									if (!a.grid.hDiv.loading)
									{
										...
										if (a.p.grouping)
										{
											...
											if (a.p.groupingView.groupDataSorted === true)
												h[o.sort] = a.p.groupingView.groupField[0] + " " + a.p.groupingView.groupOrder[0] + ", " + h[o.sort]
										}
								...
								u = function(d, g)
								{
									...
									b("#" + d)
											.append(
													"<div id='"
															...
															+ w + "' align='left'></td><td id='" + C
															+ "' align='center' style='white-space:pre;'></td><td id='" + D
									...
									if (a.p.rowList.length > 0)
									{
										...
										for (w = 0; w < a.p.rowList.length; w++)
											o += '<option role="option" value="' + a.p.rowList[w] + '"'
													+ (a.p.rowNum == a.p.rowList[w] ? ' selected="selected"' : "") + ">" + a.p.rowList[w] + "</option>";
										o += "</select></td>"
									}
									if (l == "rtl")
										q += o;
									...
									a.p.viewrecords === true
											&& b("td#" + d + "_" + a.p.recordpos, "#" + x).append(
													"<div dir='" + l + "' style='text-align:" + a.p.recordpos + "' class='ui-paging-info'></div>");
									b("td#" + d + "_" + a.p.pagerpos, "#" + x).append(q);
									o = b(".ui-jqgrid").css("font-size") || "11px";
									b(document.body).append(
											"<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:" + o
								...
								}, Ga = function(d)
								{
									...
									if (b(a.grid.cDiv).is(":visible"))
										h[1] += b(a.grid.cDiv).height() + parseInt(b(a.grid.cDiv).css("padding-top"), 10)
												+ parseInt(b(a.grid.cDiv).css("padding-bottom"), 10);
									if (a.p.toolbar[0] === true && (a.p.toolbar[1] == "top" || a.p.toolbar[1] == "both"))
										h[1] += b(a.grid.uDiv).height() + parseInt(b(a.grid.uDiv).css("border-top-width"), 10)
												+ parseInt(b(a.grid.uDiv).css("border-bottom-width"), 10);
									if (a.p.toppager)
								...
								na = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc" + na
										+ " ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-" + l + "'></span>";
								na += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + ma
										+ " ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-" + l + "'></span></span>";
								for (k = 0; k < this.p.colNames.length; k++)
								{
									ma = a.p.headertitles ? ' title="' + b.jgrid.stripHtml(a.p.colNames[k]) + '"' : "";
									aa += "<th id='" + a.p.id + "_" + a.p.colModel[k].name
											+ "' role='columnheader' class='ui-state-default ui-th-column ui-th-" + l + "'" + ma + ">";
								...
								if (this.p.multiselect)
								{
									...
									b("#cb_" + b.jgrid.jqID(a.p.id), this).bind("click", function()
									{
										if (this.checked)
										{
											b("[id^=jqg_" + a.p.id + "_]").attr("checked", true);
										...
										{
											b("[id^=jqg_" + a.p.id + "_]").attr("checked", false);
											b(a.rows).each(function(d)
											{
								...
								(function()
								{
									...
									if (a.p.shrinkToFit === true && h > 0)
									{
										...
										b.each(a.p.colModel, function(E)
										{
											...
											{
												this.width = x = Math.round(C * this.width / (a.p.tblwidth - D));
												d += x;
												q = E
											}
										});
								...
								wa = null;
								r = b(
										"<table class='ui-jqgrid-htable' style='width:" + a.p.tblwidth + "px' role='grid' aria-labelledby='gbox_" + this.id
												+ "' cellspacing='0' cellpadding='0' border='0'></table>").append(aa);
								...
								b(a).before(e.hDiv).click(
										function(d)
										{
											...
											if (W.tagName == "A"
													|| (W.tagName == "INPUT" || W.tagName == "TEXTAREA" || W.tagName == "OPTION" || W.tagName == "SELECT")
													&& !g)
												return this;
											if (h === true)
											{
								...
								b(e.bDiv).append(
										b('<div style="position:relative;' + (B && b.browser.version < 8 ? "height:0.01%;" : "") + '"></div>').append(
								...
								var Aa = a.p.hidegrid === true ? b("<a role='link' href='javascript:void(0)'/>").addClass(
								...
								b(e.cDiv).append(Aa).append("<span class='ui-jqgrid-title" + (l == "rtl" ? "-rtl" : "") + "'>" + a.p.caption + "</span>")
									...
									if (s)
									{
										...
										if (a.p.pgbuttons === true)
										{
											...
											if (h == 1 || h === 0)
											{
												...
												a.p.toppager
														&& b("#first_t, #prev_t", a.p.toppager).addClass("ui-state-disabled").removeClass("ui-state-hover")
											} else
											{
		...
		setSelection : function(f, j)
		{
			return this.each(function()
			{
				function i(l)
				{
					...
					if (u + l >= a + r)
						b(c.grid.bDiv)[0].scrollTop = u - (a + r) + l + r;
					else if (u < a + r)
						if (u < r)
					...
					try
					{
						b(this.p.colModel).each(
								function(J)
								{
									...
									if (j[c] !== undefined)
									{
										...
										l.p.treeGrid === true && c == l.p.ExpandColumn ? b("td:eq(" + J + ") > span:first", r).html(a).attr(k) : b(
												"td:eq(" + J + ")", r).html(a).attr(k)
									}
								});
						if (l.p.datatype == "local")
						{
							...
							if (typeof G != "undefined")
								l.p.data[G] = b.extend(true, l.p.data[G], B);
							B = null
						}
					} catch (Q)
					{
		...
		addRowData : function(f, j, i, c)
		{
			...
			if (j)
			{
				...
				this.each(function()
				{
					...
					m = t.p.altclass;
					for ( var S = 0, Y = "", O = {}, fa = b.isFunction(t.p.afterInsertRow) ? true : false; S < H;)
					{
						p = j[S];
						l = "";
						if (M)
						...
						if (r)
							l += b(t).jqGrid("addSubGridCell", a + u, 1);
						...
						if (t.rows.length === 0)
							b("table:first", t.grid.bDiv).append(l);
						else
							switch (i)
							{
							case "last":
								b(t.rows[t.rows.length - 1]).after(l);
								break;
							case "first":
								b(t.rows[0]).after(l);
								break;
							case "after":
								if (B = t.rows.namedItem(c))
									b(t.rows[B.rowIndex + 1]).hasClass("ui-subgrid") ? b(t.rows[B.rowIndex + 1]).after(l) : b(B).after(l);
								break;
							case "before":
								if (B = t.rows.namedItem(c))
								{
					...
					if (t.p.altRows === true && !M)
						if (i == "last")
							(t.rows.length - 1) % 2 == 1 && b(t.rows[t.rows.length - 1]).addClass(m);
						else
							b(t.rows).each(function(P)
							{
								P % 2 == 1 ? b(this).addClass(m) : b(this).removeClass(m)
							});
			...
			this.each(function()
			{
				...
				b(this.p.colModel).each(function(B)
				{
					...
					if (f == "set")
					{
						...
						l[e] = b("tr.footrow td:eq(" + B + ")", r.grid.sDiv).html()
				})
			});
			return f == "get" ? l : k
		},
				...
				if (i.grid)
				{
					...
					b(this.p.colModel).each(function(k)
					{
						if (b.inArray(this.name, f) !== -1 && this.hidden === e)
						{
							...
							b(i.rows).each(function(l)
							{
								b("td:eq(" + k + ")", i.rows[l]).css("display", j)
							});
					...
					if (c === true)
					{
						...
						i.p.shrinkToFit === true && b(i).jqGrid("setGridWidth", i.grid.width - 0.0010, true)
					}
				}
			})
		},
		...
		setLabel : function(f, j, i, c)
		{
			return this.each(function()
			{
				...
				if (e.grid)
				{
					...
					if (k >= 0)
					{
						var l = b("tr.ui-jqgrid-labels th:eq(" + k + ")", e.grid.hDiv);
						if (j)
						{
							var a = b(".s-ico", l);
						...
						if (i)
							typeof i === "string" ? b(l).addClass(i) : b(l).css(i);
				...
				if (l.grid)
				{
					...
					if (a >= 0)
					{
						...
						if (r && r > 0)
						{
							for (; u < r;)
							{
								if (b(l.rows[u]).hasClass("jqgrow"))
								{
									try
									{
										e = b.unformat(b(l.rows[u].cells[a]), {
											rowId : l.rows[u].id,
											colModel : l.p.colModel[a]
										}, a)
									} catch (B)
									{
										e = b.jgrid.htmlDecode(l.rows[u].cells[a].innerHTML)
									}
									if (i)
										k += parseFloat(e);
									else if (j)
...
(function(c)
{
	...
	c.fmatter.util = {
		...
		DateFormat : function(a, b, d, e)
		{
			...
			var l = f.getSeconds(), o = f.getMilliseconds(), n = f.getDay(), p = f.getFullYear(), q = (n + 6) % 7 + 1, s = (new Date(p, i - 1, b) - new Date(p,
					0, 1)) / 864E5, t = {
				d : h(b),
				D : k.i18n.dayNames[n],
				j : b,
	...
	c.fn.fmatter.date = function(a, b, d, e)
	{
		d = c.extend({}, b.date);
		c.fmatter.isUndefined(b.colModel.formatoptions) || (d = c.extend({}, d, b.colModel.formatoptions));
		return !d.reformatAfterEdit && e == "edit" ? c.fn.fmatter.defaultFormat(a, b) : c.fmatter.isEmpty(a) ? c.fn.fmatter.defaultFormat(a, b)
	...
	c.fn.fmatter.actions = function(a, b)
	{
		...
		if (a.editbutton)
		{
			...
			e = e + "<div style='margin-left:8px;'><div title='" + c.jgrid.nav.edittitle
		...
		if (a.delbutton)
		{
			...
			e = e + "<div title='" + c.jgrid.nav.deltitle + "' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' " + h
					+ "><span class='ui-icon ui-icon-trash'></span></div>"
		}
		h = "onclick=$.fn.fmatter.rowactions('" + d + "','" + b.gid + "','save',false);";
		e = e + "<div title='" + c.jgrid.edit.bSubmit
				+ "' style='float:left;display:none' class='ui-pg-div ui-inline-save'><span class='ui-icon ui-icon-disk' " + h + "></span></div>";
		h = "onclick=$.fn.fmatter.rowactions('" + d + "','" + b.gid + "','cancel',false);";
		return e = e + "<div title='" + c.jgrid.edit.bCancel
				+ "' style='float:left;display:none;margin-left:5px;' class='ui-pg-div ui-inline-cancel'><span class='ui-icon ui-icon-cancel' " + h
				+ "></span></div></div>"
	};
	c.unformat = function(a, b, d, e)
	{
		var h, g = b.colModel.formatter, f = b.colModel.formatoptions || {}, i = /([\.\*\_\'\(\)\{\}\+\?\\])/g, j = b.colModel.unformat || c.fn.fmatter[g]
				&& c.fn.fmatter[g].unformat;
			...
			return this.each(function()
			{
				...
				if (n.grid)
				{
					...
					for ( var q = 0; q < n.p.colModel.length; q++)
						if (n.p.colModel[q].index == f || n.p.colModel[q].name == f)
						{
							s = q;
							break
						}
									...
									if (o = h.rows.namedItem(c[d]))
									{
										...
										if (b === true)
											if (h.p.jsonReader.repeatitems === true)
											{
												...
												{
													l = h.formatter(g, c[e], e, c, "edit");
													q = h.p.colModel[e].title ? {
														title : a.jgrid.stripHtml(l)
													} : {};
													h.p.treeGrid === true && n == h.p.ExpandColumn ? a("td:eq(" + e + ") > span:first", o).html(l).attr(q) : a(
															"td:eq(" + e + ")", o).html(l).attr(q)
												}
												return s = true
											}
										a(h.p.colModel).each(
												function(m)
												{
													n = b === true ? this.jsonmap || this.name : this.name;
													if (c[n] !== undefined)
													{
														l = h.formatter(g, c[n], m, c, "edit");
														q = this.title ? {
															title : a.jgrid.stripHtml(l)
														} : {};
														h.p.treeGrid === true && n == h.p.ExpandColumn ? a("td:eq(" + m + ") > span:first", o).html(l).attr(q)
																: a("td:eq(" + m + ")", o).html(l).attr(q);
														s = true
													}
												})
									}
			...
			return this.each(function()
			{
				...
				else if (f)
				{
					...
					if (n)
					{
						if (this.p.gridModel === true)
						{
							...
							a.each(b.p.filterModel, function()
							{
								for ( var g = 0; g < n.length; g++)
									...
									{
										this.index = n[g].index || this.name;
										break
									}
								if (!this.index)
						...
						(function()
						{
							var g = document.createElement("tr"), e, m, i, k;
							...
							a.each(b.p.filterModel, function(p)
							{
								...
								case "select":
									if (this.surl)
										a(k).load(this.surl, function()
										{
											...
											b.p.gridToolbar === true && j.width && a("select", this).width(j.width);
								...
								case "text":
									...
									if (b.p.gridToolbar === true && j.width)
										a.browser.msie ? a("input", k).width(j.width - 4) : a("input", k).width(j.width - 2);
								...
								{
									b.p.gridToolbar === true && b.p.gridNames === false ? a(g).append(k) : a(g).append(i).append(k);
									a(g).append(k)
								} else
								{
							...
							{
								m = "<input type='button' id='sButton' class='" + b.p.buttonclass + "' value='" + b.p.searchButton + "'/>";
								a(k).append(m);
			...
			return this.each(function()
			{
				...
				var b = this, n = function()
				{
					...
					a.each(b.p.colModel, function()
					{
						...
						var j = this.searchoptions && this.searchoptions.sopt ? this.searchoptions.sopt[0] : f.defaultSearch;
						switch (this.stype)
						{
						case "select":
					...
					{
						var m = '{"groupOp":"' + f.groupOp + '","rules":[', i = 0;
				...
				a.each(b.p.colModel, function()
				{
					...
					if (this.search)
								...
								a.ajax(a.extend({
									...
									complete : function(p)
									{
										if (c.buildSelect !== undefined)
											(p = c.buildSelect(p)) && a(l).append(p);
										else
											a(l).append(p.responseText);
										...
										f.autosearch === true && a("select", l).change(function()
										{
											n();
											return false
										});
						...
						case "text":
							...
							a(l).append(
									"<input type='text' style='width:95%;padding:0px;' name='" + (h.index || h.name) + "' id='gs_" + h.name + "' value='" + g
											+ "'/>");
							...
							if (f.autosearch === true)
								f.searchOnEnter ? a("input", l).keypress(function(p)
								{
				...
				this.clearToolbar = function(h)
				{
					...
					a.each(b.p.colModel, function()
					{
						...
						case "select":
							var j;
							a("select[name=" + g + "] option", b.grid.hDiv).each(function(t)
							{
								if (t === 0)
									this.selected = true;
								if (a(this).text() == o)
...
}, viewModal = function(a, b)
{
	...
	if (jQuery.fn.jqm && b.jqm === true)
		b.jqM ? jQuery(a).attr("aria-hidden", "false").jqm(b).jqmShow() : jQuery(a).attr("aria-hidden", "false").jqmShow();
	else
	{
		if (b.gbox !== "")
		{
...
function info_dialog(a, b, c, e)
{
	...
	d += "<div id='infocnt' style='margin:0px;padding-bottom:1em;width:100%;overflow:auto;position:relative;height:" + e + ";"
			+ ("text-align:" + f.align + ";") + "'>" + b + "</div>";
	d += c ? "<div class='ui-widget-content ui-helper-clearfix' style='text-align:"
			...
			+ c + "</a>" + a + "</div>"
			: a !== "" ? "<div class='ui-widget-content ui-helper-clearfix' style='text-align:" + f.buttonalign
					+ ";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>" + a + "</div>" : "";
	d += "</div>";
	try
	{
...
function createEl(a, b, c, e, f)
{
	...
	case "textarea":
		...
		if (!b.rows)
			b.rows = 2;
		if (c == "&nbsp;" || c == "&#160;" || c.length == 1 && c.charCodeAt(0) == 160)
			c = "";
		d.value = c;
		b = g(d, b);
		...
		else if (b.value)
		{
			...
			if (typeof b.value === "string")
			{
				...
				for (h = 0; h < e.length; h++)
				{
					...
					a.value = f[0];
					a.innerHTML = f[1];
					if (!j && (jQuery.trim(f[0]) == jQuery.trim(c) || jQuery.trim(f[1]) == jQuery.trim(c)))
						a.selected = "selected";
					if (j && (jQuery.inArray(jQuery.trim(f[1]), l) > -1 || jQuery.inArray(jQuery.trim(f[0]), l) > -1))
						a.selected = "selected";
					d.appendChild(a)
				}
			} else if (typeof b.value === "object")
			{
				...
				for (h in e)
					if (e.hasOwnProperty(h))
					{
						...
						a.value = h;
						a.innerHTML = e[h];
						if (!j && (jQuery.trim(h) == jQuery.trim(c) || jQuery.trim(e[h]) == jQuery.trim(c)))
							a.selected = "selected";
						if (j && (jQuery.inArray(jQuery.trim(e[h]), l) > -1 || jQuery.inArray(jQuery.trim(h), l) > -1))
							a.selected = "selected";
						d.appendChild(a)
					}
			}
	...
	case "custom":
		...
		} catch (n)
		{
			...
			n == "e2" ? info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_element' " + jQuery.jgrid.edit.msg.novalue, jQuery.jgrid.edit.bClose)
					: info_dialog(jQuery.jgrid.errors.errcap, typeof n === "string" ? n : n.message, jQuery.jgrid.edit.bClose)
		}
		break
	}
	return d
...
function checkDate(a, b)
{
	...
	{
		i = c[a[g]].toString();
		if (i.length < 1 || c[a[g]] < 1 || c[a[g]] > 31 || c[a[d]] == 2 && c[a[g]] > daysInFebruary(c[a[e]]) || c[a[g]] > h[c[a[d]]])
			return false
	}
	return true
}
	...
	if (e)
	{
		...
		if (typeof e.minValue != "undefined" && !isNaN(e.minValue))
			if (parseFloat(a) < parseFloat(e.minValue))
				return [ false, g + ": " + jQuery.jgrid.edit.msg.minValue + " " + e.minValue, "" ];
		if (typeof e.maxValue != "undefined" && !isNaN(e.maxValue))
			if (parseFloat(a) > parseFloat(e.maxValue))
				return [ false, g + ": " + jQuery.jgrid.edit.msg.maxValue + " " + e.maxValue, "" ];
		if (e.email === true)
			if (!(f === false && isEmpty(a)))
			{
		...
		if (e.date === true)
			if (!(f === false && isEmpty(a)))
			{
				b = c.p.colModel[b].formatoptions && c.p.colModel[b].formatoptions.newformat ? c.p.colModel[b].formatoptions.newformat
		...
		if (e.time === true)
			if (!(f === false && isEmpty(a)))
				if (!checkTime(a))
					return [ false, g + ": " + jQuery.jgrid.edit.msg.date + " - hh:mm (am/pm)", "" ];
		if (e.url === true)
			if (!(f === false && isEmpty(a)))
			{
							...
							.each(function()
							{
								function b(m, q)
								{
									...
									if (q)
									{
										...
										if (q.rules)
										{
											...
											{
												A = q.rules[F];
												if (A.field !== undefined && A.op !== undefined && A.data !== undefined)
								...
								if (w.grid)
								{
									...
									if (a.fn.searchFilter)
									{
										...
										{
											var n = [], E = a("#" + w.p.id).jqGrid("getGridParam", "colNames"), g = a("#" + w.p.id).jqGrid("getGridParam",
													"colModel"), k = [ "eq", "ne", "lt", "le", "gt", "ge", "bw", "bn", "in", "ni", "ew", "en", "cn", "nc" ], h, j, l, s = [];
											if (f.sopt !== null)
											...
											{
												var A = typeof q.search === "undefined" ? true : q.search, F = q.hidden === true;
												m = a.extend({}, {
													text : E[m],
											...
											if (n.length > 0)
											{
												...
												if (f.drag === true)
												{
													...
													if (jQuery.fn.jqDrag)
														a("#" + i).jqDrag(a("#" + i + " table thead tr:first td:first"));
													else
														try
														{
															a("#" + i).draggable({
				...
				editGridRow : function(f, b)
				{
					...
					return this
							.each(function()
							{
												...
												if (e.length)
												{
													...
													a.each(g.p.colModel, function()
													{
														if (this.name == r && this.editoptions && a.isFunction(this.editoptions.custom_value))
														{
															...
															{
																p == "e1" ? info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_value' "
																		+ a.jgrid.edit.msg.novalue, jQuery.jgrid.edit.bClose) : info_dialog(
																		jQuery.jgrid.errors.errcap, p.message, jQuery.jgrid.edit.bClose)
															}
															return true
														}
													})
								...
								function C(e, r, t, p)
								{
									...
									a(r.p.colModel)
											.each(
													...
													{
														I = this.name;
														L = (z = this.editrules && this.editrules.edithidden === true ? false : this.hidden === true ? true
																: false) ? "style='display:none'" : "";
														if (I !== "cb" && I !== "subgrid" && this.editable === true && I !== "rn")
														{
															if (H === false)
																y = "";
															else if (I == r.p.ExpandColumn && r.p.treeGrid === true)
															...
															var W = a.extend({}, this.editoptions || {}, {
																...
																rowabove : false,
																rowcontent : ""
															}, this.formoptions || {}), aa = parseInt(X.rowpos, 10) || J + 1, ea = parseInt((parseInt(X.colpos,
																	10) || 1) * 2, 10);
															if (e == "_empty" && W.defaultValue)
																y = a.isFunction(W.defaultValue) ? W.defaultValue() : W.defaultValue;
															if (!this.edittype)
																this.edittype = "text";
															if (g.p.autoencode)
									...
									if (J > 0)
									{
										S = a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='" + (p * 2 - 1)
												+ "' class='DataTD'><input class='FormElement' id='id_g' type='text' name='" + r.p.id + "_id' value='" + e
												+ "'/></td></tr>");
										S[0].rp = J + 999;
										a(t).append(S);
										if (d.checkOnSubmit || d.checkOnUpdate)
										...
										a(G).each(function()
										{
											...
											if (x[0] != null)
											{
												...
												} else if (x[0].type == "checkbox")
												{
													...
													y = a(x).attr("offval")
												} else if (x[0].type && x[0].type.substr(0, 6) == "select")
													x[0].selectedIndex = 0;
												else
													x.val(y);
										...
										if (R)
										{
											a("td", R).each(
													function(H)
													{
														...
														{
															if (p == r.p.ExpandColumn && r.p.treeGrid === true)
																z = a(this).text();
															else
																try
																{
																...
																a("#" + p + " option", "#" + t).each(
																		function()
																		{
																			this.selected = !G[H].editoptions.multiple
																					&& (S[0] == a.trim(a(this).text()) || S[0] == a.trim(a(this).val())) ? true
																					: G[H].editoptions.multiple ? a.inArray(a.trim(a(this).text()), S) > -1
																							|| a.inArray(a.trim(a(this).val()), S) > -1 ? true : false : false
																		});
																break;
															case "checkbox":
																z += "";
																if (G[H].editoptions && G[H].editoptions.value)
															...
															case "custom":
																try
																{
																	if (G[H].editoptions && a.isFunction(G[H].editoptions.custom_value))
																		G[H].editoptions.custom_value(a("#" + p, "#" + t), "set", z);
																	else
																		throw "e1";
																} catch (T)
																{
																	T == "e1" ? info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_value' "
																			+ a.jgrid.edit.msg.nodefined, jQuery.jgrid.edit.bClose) : info_dialog(
																			jQuery.jgrid.errors.errcap, T.message, jQuery.jgrid.edit.bClose)
																}
																break
															}
															I++
									...
									if (e[0] && !d.processing)
									{
										...
										r = a.extend({
											...
											complete : function(J, y)
											{
												...
												{
													e[0] = false;
													e[1] = a.isFunction(d.errorTextFormat) ? d.errorTextFormat(J) : y + " Status: '" + J.statusText
															+ "'. Error code: " + J.status
												...
												} else
												{
													a.each(g.p.colModel, function()
													{
														if (u[this.name] && this.formatter && this.formatter == "select")
															try
															{
																delete u[this.name]
															} catch (R)
													...
													} else
													{
														if (d.reloadAfterSubmit)
														{
															...
															g.p.treeGrid === true ? a(g).jqGrid("setTreeRow", c[p], c) : a(g).jqGrid("setRowData", c[p], c);
										...
										if (e[0])
											d.useDataProxy ? g.p.dataProxy.call(g, r, "set_" + g.p.id) : a.ajax(r)
									}
									if (e[0] === false)
									{
										a("#FormError>td", "#" + j).html(e[1]);
								...
								function n(e, r)
								{
									e === 0 ? a("#pData", "#" + j + "_2").addClass("ui-state-disabled") : a("#pData", "#" + j + "_2").removeClass(
											"ui-state-disabled");
									e == r ? a("#nData", "#" + j + "_2").addClass("ui-state-disabled") : a("#nData", "#" + j + "_2").removeClass(
											"ui-state-disabled")
								}
								function E()
								{
								...
								if (g.grid && f)
								{
									var k = g.p.id, h = "FrmGrid_" + k, j = "TblGrid_" + k, l = {
										...
										scrollelm : h
									}, s = a.isFunction(d.beforeShowForm) ? d.beforeShowForm : false, Q = a.isFunction(d.afterShowForm) ? d.afterShowForm
											: false, m = a.isFunction(d.beforeInitData) ? d.beforeInitData : false, q = a.isFunction(d.onInitializeForm) ? d.onInitializeForm
									...
									if (a("#" + l.themodal).html() != null)
									{
										...
										m && m(a("#" + h));
										v(f, g, h);
										f == "_empty" || !d.viewPagerButtons ? a("#pData, #nData", "#" + j + "_2").hide() : a("#pData, #nData", "#" + j + "_2")
												.show();
									...
									} else
									{
										...
										var Z = "<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default ui-corner-all'>" + b.bSubmit
												+ "</a>", $ = "<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"
												+ b.bCancel + "</a>";
										D = "<table border='0' class='EditTable' id='" + j
												+ "_2'><tbody><tr id='Act_Buttons'><td class='navButton ui-widget-content'>" + (m ? V + D : D + V)
												+ "</td><td class='EditButton ui-widget-content'>" + Z + $ + "</td></tr>";
										...
										if (b.saveicon[0] === true)
											a("#sData", "#" + j + "_2").addClass(b.saveicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left")
													.append("<span class='ui-icon " + b.saveicon[2] + "'></span>");
										if (b.closeicon[0] === true)
											a("#cData", "#" + j + "_2").addClass(b.closeicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left")
										...
										if (d.checkOnSubmit || d.checkOnUpdate)
										{
											...
											a(
													"<div class='ui-widget-overlay jqgrid-overlay confirm' style='z-index:"
															...
															+ (P + 1) + "'>" + b.saveData + "<br/><br/>" + Z + V + $ + "</div>").insertAfter("#" + h);
							...
							.each(function()
							{
								...
								function C(c, u, M, U)
								{
									for ( var O, P, N, D = 0, V, Z, $ = [], Y = false, ba = "<td class='CaptionTD form-view-label ui-widget-content' width='"
											+ b.labelswidth + "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>", e = "", r = [
									...
									a(u.p.colModel).each(function()
									{
										...
										if (!P && this.align === "right")
											if (this.formatter && a.inArray(this.formatter, r) !== -1)
									...
									a(u.p.colModel)
											.each(
													function(y)
													{
														O = this.name;
														z = false;
														Z = (P = this.editrules && this.editrules.edithidden === true ? false : this.hidden === true ? true
																: false) ? "style='display:none'" : "";
														x = typeof this.viewable != "boolean" ? true : this.viewable;
														if (O !== "cb" && O !== "subgrid" && O !== "rn" && x)
														{
															V = Y === false ? "" : O == u.p.ExpandColumn && u.p.treeGrid === true ? a("td:eq(" + y + ")",
																	u.rows[Y]).text() : a("td:eq(" + y + ")", u.rows[Y]).html();
															...
															var L = a.extend({}, {
																rowabove : false,
																rowcontent : ""
															}, this.formoptions || {}), G = parseInt(L.rowpos, 10) || D + 1, R = parseInt((parseInt(L.colpos,
																	10) || 1) * 2, 10);
															if (L.rowabove)
															{
															...
															a("td:eq(" + (R - 2) + ")", N[0]).html(
																	"<b>" + (typeof L.label === "undefined" ? u.p.colNames[y] : L.label) + "</b>");
									...
									if (D > 0)
									{
										c = a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='" + (U * 2 - 1)
												+ "' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='" + c + "'/></td></tr>");
										c[0].rp = D + 99;
										a(M).append(c)
									}
									return $
									...
									if (N = a(u).jqGrid("getInd", c, true))
									{
										a("td", N).each(
												function(D)
												{
													...
													U = u.p.colModel[D].editrules && u.p.colModel[D].editrules.edithidden === true ? false
															: u.p.colModel[D].hidden === true ? true : false;
								...
								function B(c, u)
								{
									c === 0 ? a("#pData", "#" + g + "_2").addClass("ui-state-disabled") : a("#pData", "#" + g + "_2").removeClass(
											"ui-state-disabled");
									c == u ? a("#nData", "#" + g + "_2").addClass("ui-state-disabled") : a("#nData", "#" + g + "_2").removeClass(
											"ui-state-disabled")
								}
								function w()
								{
								...
								if (i.grid && f)
								{
									...
									} else
									{
										...
										var l = isNaN(b.dataheight) ? b.dataheight : b.dataheight + "px", s = a("<form name='FormPost' id='" + E
												+ "' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:" + l + ";'></form>"), Q = a("<table id='"
												+ g
										...
										s = a("<span></span>").append(s).append(
												"<table border='0' class='EditTable' id='" + g
														+ "_2'><tbody><tr id='Act_Buttons'><td class='navButton ui-widget-content' width='" + b.labelswidth
														+ "'>" + (l ? q + m : m + q) + "</td><td class='EditButton ui-widget-content'>" + A
														+ "</td></tr></tbody></table>");
										...
										b.closeicon = a.extend([ true, "left", "ui-icon-close" ], b.closeicon);
										if (b.closeicon[0] === true)
											a("#cData", "#" + g + "_2").addClass(b.closeicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left")
													.append("<span class='ui-icon " + b.closeicon[2] + "'></span>");
										a.isFunction(b.beforeShowForm) && b.beforeShowForm(a("#" + E));
										viewModal("#" + k.themodal, {
											gbox : "#gbox_" + n,
					...
					return this
							.each(function()
							{
								...
								if (o.grid)
									if (f)
									{
										var C = typeof b.beforeShowForm === "function" ? true : false, v = typeof b.afterShowForm === "function" ? true : false, B = o.p.id, w = {}, i = "DelTbl_"
												+ B, n, E, g, k, h = {
											themodal : "delmod" + B,
											modalhead : "delhd" + B,
										...
										} else
										{
											...
											j += "<table cellspacing='0' cellpadding='0' border='0' class='EditTable' id='"
													...
													+ ("<a href='javascript:void(0)' id='dData' class='fm-button ui-state-default ui-corner-all'>" + b.bSubmit + "</a>")
													+ "&#160;"
													+ ("<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default ui-corner-all'>" + b.bCancel + "</a>")
													+ "</td></tr></tbody></table>";
											b.gbox = "#gbox_" + B;
											...
											if (b.delicon[0] === true)
												a("#dData", "#" + i + "_2").addClass(b.delicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left")
														.append("<span class='ui-icon " + b.delicon[2] + "'></span>");
											if (b.cancelicon[0] === true)
												a("#eData", "#" + i + "_2").addClass(
														b.cancelicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left").append(
														"<span class='ui-icon " + b.cancelicon[2] + "'></span>");
											a("#dData", "#" + i + "_2").click(
													function()
													{
														var l = [ true, "" ];
														...
														if (l[0] && !d.processing)
														{
															...
															var Q = a.extend({
																...
																complete : function(m, q)
																{
																	...
																	{
																		l[0] = false;
																		l[1] = a.isFunction(d.errorTextFormat) ? d.errorTextFormat(m) : q + " Status: '"
															...
															if (l[0])
																d.useDataProxy ? o.p.dataProxy.call(o, Q, "del_" + o.p.id) : a.ajax(Q)
														}
														if (l[0] === false)
														{
															a("#DelError>td", "#" + i).html(l[1]);
							...
							.each(function()
							{
								...
								if (!(!n.grid || typeof f != "string"))
								{
									...
									for (g = 0; g < E; g++)
									{
												...
												id : C.id || "add_" + l
													...
													function()
													{
														a(this).hasClass("ui-state-disabled")
																|| (typeof b.addfunc == "function" ? b.addfunc() : a(n).jqGrid("editGridRow", "new", C));
														return false
													}).hover(function()
											{
										...
										if (b.edit)
										{
											...
											}).click(function()
											{
												if (!a(this).hasClass("ui-state-disabled"))
												{
													var s = n.p.selrow;
													if (s)
														typeof b.editfunc == "function" ? b.editfunc(s) : a(n).jqGrid("editGridRow", s, o);
													else
													{
											...
											}).click(function()
											{
												if (!a(this).hasClass("ui-state-disabled"))
												{
													...
													if (n.p.multiselect)
													{
														...
														s = n.p.selrow;
													if (s)
														"function" == typeof b.delfunc ? b.delfunc(s) : a(n).jqGrid("delGridRow", s, v);
													else
													{
														viewModal("#" + i.themodal, {
															gbox : "#gbox_" + n.p.id,
										...
										a("body").append(
												"<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:" + k
														+ ";visibility:hidden;' ></div>");
				...
				navButtonAdd : function(f, b)
				{
					...
					return this.each(function()
					{
						if (this.grid)
						{
							...
							{
								var v = a("<td></td>");
								b.buttonicon.toString().toUpperCase() == "NONE" ? a(v).addClass("ui-pg-button ui-corner-all").append(
										"<div class='ui-pg-div'>" + b.caption + "</div>") : a(v).addClass("ui-pg-button ui-corner-all").append(
										"<div class='ui-pg-div'><span class='ui-icon " + b.buttonicon + "'></span>" + b.caption + "</div>");
								b.id && a(v).attr("id", b.id);
								if (b.position == "first")
									o.rows[0].cells.length === 0 ? a("tr", o).append(v) : a("tr td:eq(0)", o).before(v);
								else
									a("tr", o).append(v);
				...
				navSeparatorAdd : function(f, b)
				{
					...
					return this.each(function()
					{
						if (this.grid)
						{
							...
							if (o)
							{
								var C = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='" + b.sepclass + "'></span>" + b.sepcontent
										+ "</td>";
								a("tr", o).append(C)
							}
						}
				...
				GridToForm : function(f, b)
				{
					return this.each(function()
					{
						...
						if (o.grid)
						{
							...
							if (C)
								for ( var v in C)
									a("[name=" + v + "]", b).is("input:radio") || a("[name=" + v + "]", b).is("input:checkbox") ? a("[name=" + v + "]", b)
											.each(function()
											{
			...
			e.find(".ui-add").click(function(a)
			{
				...
				if (f.clone)
				{
					...
					if (b.find("select[name='op']")[0] != null)
						b.find("select[name='op']").focus()[0].selectedIndex = a.find("select[name='op']")[0].selectedIndex;
			...
			e.find(".ui-search").click(
					function()
					{
						...
						a.find(".sf").each(
								function(c)
								{
									var g = jQuery(this).find("select[name='field'] :selected").val(), i = jQuery(this).find("select[name='op'] :selected")
											.val(), h = jQuery(this).find("input.vdata,select.vdata :selected").val();
									h += "";
		...
		editRow : function(d, u, i, n, o, r, s, c, f)
		{
			return this.each(function()
			{
				...
				if (b.grid)
				{
					...
					if (h !== false)
						if ((a(h).attr("editable") || "0") == "0" && !a(h).hasClass("not-editable-row"))
						{
							g = b.p.colModel;
									...
									function(m)
									{
										...
										if (f != "cb" && f != "subgrid" && h.editable === true && f != "rn")
										{
											...
											case "custom":
												...
												{
													w == "e1"
															&& info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_value' " + a.jgrid.edit.msg.nodefined,
																	jQuery.jgrid.edit.bClose);
													w == "e2" ? info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_value' " + a.jgrid.edit.msg.novalue,
															jQuery.jgrid.edit.bClose) : info_dialog(jQuery.jgrid.errors.errcap, w.message,
															jQuery.jgrid.edit.bClose)
												}
												break
											}
								...
								} else
								{
									...
									a.ajax(a.extend({
										...
										complete : function(m, e)
										{
											a("#lui_" + c.p.id).hide();
											if (e === "success")
												if ((a.isFunction(u) ? u.call(c, m) : true) === true)
												{
										...
										{
											a("#lui_" + c.p.id).hide();
											a.isFunction(r) ? r.call(c, d, m, e) : alert("Error Row: " + d + " Result: " + m.status + ":" + m.statusText
													+ " Status: " + e);
											a(c).jqGrid("restoreRow", d, s)
										}
											...
											function(l)
											{
												if (l.keyCode === 27)
													if (b("input.hasDatepicker", g).length > 0)
														b(".ui-datepicker").is(":hidden") ? b(c).jqGrid("restoreCell", d, e) : b("input.hasDatepicker", g)
																.datepicker("hide");
													else
												...
												if (l.keyCode == 9)
													if (c.grid.hDiv.loading)
														...
														l.shiftKey ? b(c).jqGrid("prevCell", d, e) : b(c).jqGrid("nextCell", d, e);
												l.stopPropagation()
											});
						...
						if (!(!a.grid || a.p.cellEdit !== true))
						{
							...
							if (c !== null)
							{
								...
								case "custom":
									...
									{
										q == "e1"
												&& info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_value' " + b.jgrid.edit.msg.nodefined,
														jQuery.jgrid.edit.bClose);
										q == "e2" ? info_dialog(jQuery.jgrid.errors.errcap, "function 'custom_value' " + b.jgrid.edit.msg.novalue,
												jQuery.jgrid.edit.bClose) : info_dialog(jQuery.jgrid.errors.errcap, q.message, jQuery.jgrid.edit.bClose)
									}
									break
								}
								if (g != a.p.savedRow[c].v)
								{
									if (b.isFunction(a.p.beforeSaveCell))
										if (c = a.p.beforeSaveCell.call(a, a.rows[d].id, j, f, d, e))
											f = c;
									...
									if (r[0] === true)
									{
										...
										if (b.isFunction(a.p.beforeSubmitCell))
											(c = a.p.beforeSubmitCell.call(a, a.rows[d].id, j, f, d, e)) || (c = {});
											...
											if (a.p.cellurl)
											{
												...
												b.ajax(b.extend({
													...
													error : function(m, p)
													{
														b("#lui_" + a.p.id).hide();
														a.grid.hDiv.loading = false;
														b.isFunction(a.p.errorCell) ? a.p.errorCell.call(a, m, p) : info_dialog(b.jgrid.errors.errcap, m.status
																+ " : " + m.statusText + "<br/>" + p, b.jgrid.edit.bClose);
														b(a).jqGrid("restoreCell", d, e)
													}
							...
							.each(function()
							{
								function d(g, k, j)
								{
									...
									if (j == "h")
									{
										...
										if (a.rows[g].cells[k].offsetLeft + a.rows[g].cells[k].clientWidth >= j + parseInt(i, 10))
...
(function(b)
{
	...
	var l = 0, i = b.jqm.hash, k = [], q = b.browser.msie && b.browser.version == "6.0", e = false, r = function(a)
	{
...
(function(b)
{
	b.jgrid.extend({
		...
		addSubGridCell : function(e, c)
		{
			...
			return n === false ? '<td role="grid" aria-describedby="' + o + '_subgrid" class="ui-sgcollapsed sgcollapsed" ' + a
					+ "><a href='javascript:void(0);'><span class='ui-icon ui-icon-plus'></span></a></td>" : '<td role="grid" aria-describedby="' + o
					+ '_subgrid" ' + a + "></td>"
		},
					...
					b("td:eq(" + c + ")", e).click(
							function()
							{
								if (b(this).hasClass("sgcollapsed"))
								{
									...
									w = "<tr role='row' class='ui-subgrid'>" + v
											+ "<td class='ui-widget-content subgrid-cell'><span class='ui-icon ui-icon-carat-1-sw'/></td><td colspan='"
											+ parseInt(a.p.colNames.length - 1 - t, 10) + "' class='ui-widget-content subgrid-data'><div id=" + s + "_" + m
											+ " class='tablediv'>";
									b(this).parent().after(w + "</div></td></tr>");
									b.isFunction(a.p.subGridRowExpanded) ? a.p.subGridRowExpanded.call(a, s + "_" + m, m) : x(r);
									b(this).html("<a href='javascript:void(0);'><span class='ui-icon ui-icon-minus'></span></a>").removeClass("sgcollapsed")
											.addClass("sgexpanded")
								} else if (b(this).hasClass("sgexpanded"))
								{
									...
									b(this).html("<a href='javascript:void(0);'><span class='ui-icon ui-icon-plus'></span></a>").removeClass("sgexpanded")
...
(function(f)
{
	f.jgrid.extend({
		...
		groupingToggle : function(b)
		{
			this.each(function()
			{
				var c = this.p.groupingView, a = b.lastIndexOf("_"), d = b.substring(0, a + 1);
		...
		groupingRender : function(b, c)
		{
			return this.each(function()
			{
				...
				{
					j = a.p.id + "ghead_" + h;
					g = "<span style='cursor:pointer;' class='ui-icon " + l + "' onclick=\"jQuery('#" + a.p.id + "').jqGrid('groupingToggle','" + j
							+ "');return false;\"></span>";
					...
					return this.each(function()
					{
						...
						if (a.grid && a.p.treeGrid)
						{
							...
							if (parseInt(b[j], 10) !== parseInt(a.p.tree_root_level, 10))
								d(a).jqGrid("isVisibleNode", b) || d(c).css("display", "none");
					...
					this.each(function()
					{
						...
						if (c.grid && c.p.treeGrid)
							...
							case "adjacency":
								...
								d(c.p.data).each(function()
								{
									if (this[f] === null || String(this[f]).toLowerCase() == "null")
										b.push(this)
								});
								break
							}
					...
					this
							.each(function()
							{
								...
								if (a.grid && a.p.treeGrid)
									...
									{
									case "nested":
										var f = a.p.treeReader.left_field, e = a.p.treeReader.right_field, k = a.p.treeReader.level_field, j = parseInt(b[f],
												10), i = parseInt(b[e], 10), h = parseInt(b[k], 10);
					...
					this
							.each(function()
							{
								...
								if (a.grid && a.p.treeGrid)
									...
									{
									case "nested":
										var e = a.p.treeReader.left_field, k = a.p.treeReader.right_field, j = a.p.treeReader.level_field, i = parseInt(b[e],
												10), h = parseInt(b[k], 10), g = parseInt(b[j], 10);
										d(this.p.data).each(function()
										{
	...
	json2xml : function(a, b)
	{
		var g = function(d, c, j)
		{
			...
			else if (typeof d === "object")
			{
				...
				i += typeof d === "function" ? j + "<" + c + "><![CDATA[" + d + "]]\></" + c + ">" : d.toString() === '""' || d.toString().length === 0 ? j
						+ "<" + c + ">__EMPTY_STRING_</" + c + ">" : j + "<" + c + ">" + d.toString() + "</" + c + ">";
			return i
		}, e = "", f;
		for (f in a)
				...
				setColumns : function(a)
				{
					...
					return this
							.each(function()
							{
								...
								if (c.grid)
								{
									var j = typeof a.beforeShowForm === "function" ? true : false, k = typeof a.afterShowForm === "function" ? true : false, l = typeof a.afterSubmitForm === "function" ? true
									...
									} else
									{
										...
										for (i = 0; i < this.p.colNames.length; i++)
											c.p.colModel[i].hidedlg
													|| (g += "<tr><td style='white-space: pre;'><input type='checkbox' style='margin-right:5px;' id='col_"
															+ this.p.colModel[i].name + "' class='cbox' value='T' "
															+ (this.p.colModel[i].hidden === false ? "checked" : "") + "/><label for='col_"
															+ this.p.colModel[i].name + "'>" + this.p.colNames[i]
										...
										g += "<table border='0' class='EditTable' id='"
												...
												+ (!a.updateAfterCheck ? "<a href='javascript:void(0)' id='dData' class='fm-button ui-state-default ui-corner-all'>"
														...
														: "")
												+ "&#160;"
												+ ("<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default ui-corner-all'>" + a.bCancel + "</a>")
												+ "</td></tr></tbody></table>";
										a.gbox = "#gbox_" + e;
										createModal(f, g, a, "#gview_" + c.p.id, b("#gview_" + c.p.id)[0]);
										if (a.saveicon[0] == true)
											b("#dData", "#" + d + "_2").addClass(a.saveicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left")
													.append("<span class='ui-icon " + a.saveicon[2] + "'></span>");
										if (a.closeicon[0] == true)
											b("#eData", "#" + d + "_2").addClass(a.closeicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left")
													.append("<span class='ui-icon " + a.closeicon[2] + "'></span>");
...
(function(c)
{
	c.jgrid.extend({
		...
		appendPostData : function(a)
		{
			var b = this[0];
			if (b.grid)
				typeof a === "object" ? c.extend(b.p.postData, a) : alert("Error: cannot append a non-object postData value. postData unchanged.")
		},
...
(function(a)
{
	...
	a.jgrid.extend({
			...
			return this.each(function()
			{
				...
				var c = this, g = {
					...
					placeholder : {
						element : function(e)
						{
							return a(document.createElement(e[0].nodeName)).addClass(e[0].className + " ui-sortable-placeholder ui-state-highlight")
									.removeClass("ui-sortable-helper")[0]
						},
						update : function(e, h)
						{
							h.height(e.currentItem.innerHeight() - parseInt(e.currentItem.css("paddingTop") || 0, 10)
									- parseInt(e.currentItem.css("paddingBottom") || 0, 10));
							h.width(e.currentItem.innerWidth() - parseInt(e.currentItem.css("paddingLeft") || 0, 10)
		...
		columnChooser : function(b)
		{
			...
			{
				var d = a('<div id="colchooser_' + g[0].p.id
						+ '" style="position:relative;overflow:hidden"><div><select multiple="multiple"></select></div></div>'), e = a("select", d);
				b = a.extend({
					width : 420,
					height : 240,
					classname : null,
				...
				a.each(h, function(f)
				{
					...
					if (this.hidedlg)
						...
						e.append("<option value='" + f + "' " + (this.hidden ? "" : "selected='selected'") + ">" + i[f] + "</option>")
				});
							...
							else
							{
										...
										return a.extend({
											start : function(e, h)
											{
												...
												for (i = 0; i < a.data(c, "dnd").connectWith.length; i++)
													a(a.data(c, "dnd").connectWith[i]).jqGrid("getGridParam", "reccount") == "0"
															&& a(a.data(c, "dnd").connectWith[i]).jqGrid("addRowData", "jqg_empty_row", {});
								...
								if (b.connectWith)
								{
									...
									for ( var g = 0; g < b.connectWith.length; g++)
										a(b.connectWith[g]).droppable(a.isFunction(b.drop) ? b.drop.call(a(c), b) : b.drop)
								}
							}
						}
			})
		...
		gridResize : function(b)
		{
			return this.each(function()
			{
				...
				if (j.grid && a.fn.resizable)
				{
					...
					b.alsoResize = b._alsoResize_ ? eval("(" + ("{'#gview_" + j.p.id + " .ui-jqgrid-bdiv':true,'" + b._alsoResize_ + "':true}") + ")") : a(
